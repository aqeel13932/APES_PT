...
def SetupEnvironment():
    ...
    blue_Ag = Agent(Fname='APES/Pics/blue.jpg',
                    Power=3,
                    VisionAngle=args.svision,Range=6,
                    PdstName='blue_Ag_PM',
...
def TryModel(model,game):
    ...
    for t in range(args.max_timesteps):
        observation = np.concatenate([observation,metric])
...
ishape =(Settings.WorldSize[0]*Settings.WorldSize[1]*(2+3)+ 2*4+args.naction*5+1,)
game.GenerateWorld()
game.Step()
...
while progress<args.totalsteps:
    ...
    for t in range(args.max_timesteps):
        ...
        #If the game is not finished and dominate can see food, every step is a reward.
        if (not done) and metric:
            reward=args.rwrdschem[2]*-1

        episode_reward += reward
        mem.add(prev_ob,np.array([action]),reward,np.concatenate([observation,metric]),done)
