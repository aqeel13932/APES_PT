#In this file:
    # Dominant don't move and all orientations are passed to lstm
...
def Convlutional_output(ag):
    my = np.zeros((11,11,4))
    other = np.zeros((11,11,4))
    my_vec = np.argwhere(ag.NNFeed['myori']==1)
    other_vec = np.argwhere(ag.NNFeed['agentori1002']==1)
    if my_vec.shape[0]>0:
        my[:,:,my_vec[0][0]].fill(1)
    if other_vec.shape[0]>0:
        other[:,:,other_vec[0][0]].fill(1)
    cnn =np.stack([ag.NNFeed['observed'],
                    ag.NNFeed['mypos'],
                    ag.NNFeed['obstacles'],
                    ag.NNFeed['food'],
                    ag.NNFeed['agentpos1002']],axis=-1)
    cnn = np.dstack([cnn,my,other])
    return cnn

def createLayers(in_conv,naction):
    ...
    h = TimeDistributed(Flatten())(con_process)
...
def train_model():
    ...
    if len(mem.buffer)>=args.batch_size*2: 
        for k in range(args.train_repeat):
            ...
            qpre = model.predict_on_batch(pre_cnn)
            qpost = target_model.predict_on_batch(post_cnn)
            ...
            model.train_on_batch(pre_cnn, qpre)
...
def TryModel(model,game):
    ...
    Start = time()
    episode_reward=0
    cnn = Convlutional_output(AIAgent)
    ...
    for t in range(args.max_timesteps):
        all_cnn[t]=cnn
        q = model.predict(all_cnn[None,:], batch_size=1)
...
game.GenerateWorld()
game.Step()
conv_size=(args.max_timesteps,Settings.WorldSize[0],Settings.WorldSize[1],13,)
naction =  Settings.PossibleActions.shape[0]
rest_size=(args.max_timesteps,args.naction*5+8,)
print(conv_size,naction,rest_size)
if args.train_m=='':
    print('train default')
    c,z = createLayers(conv_size,naction)
    model = Model(inputs=c, outputs=z)
    model.summary()
    ...
    target_model = Model(inputs=c, outputs=z)
...
while progress<args.totalsteps:
    ...
    for t in range(args.max_timesteps):
        ...
        if np.random.random() < args.exploration:
            ...
            q = model.predict(pre_cnn[None,:], batch_size=1)
            action = np.argmax(q[0,t])
        lst_action[t] = action
        #Only subordinate moves, dominant is static.
        AIAgent.NextAction = Settings.PossibleActions[action]
        AIAgent.AddAction(action)
        game.Step()
        post_cnn[t]= Convlutional_output(AIAgent)
